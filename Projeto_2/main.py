...
R_L10 = "IF (Memory IS low) AND (Processor IS very_high) THEN (Load IS low)"
...
R_L20 = "IF (Memory IS high) AND (Processor IS very_high) THEN (Load IS very_low)"
R_L21 = "IF (Memory IS very_high) AND (Processor IS very_low) THEN (Load IS high)"
R_L22 = "IF (Memory IS very_high) AND (Processor IS low) THEN (Load IS low)"
R_L23 = "IF (Memory IS very_high) AND (Processor IS normal) THEN (Load IS low)"
R_L24 = "IF (Memory IS very_high) AND (Processor IS high) THEN (Load IS very_low)"
...
R_N1 = "IF (Output IS very_low) AND (Latency IS very_low) THEN (Network IS low)"
...
R_N6 = "IF (Output IS low) AND (Latency IS very_low) THEN (Network IS low"
...
R_N17 = "IF (Output IS high) AND (Latency IS very_low) THEN (Network IS very_low)"
R_N18 = "IF (Output IS high) AND (Latency IS normal) THEN (Network IS low)"
R_N19 = "IF (Output IS high) AND (Latency IS high) THEN (Network IS low)"
...
R_N22 = "IF (Output IS very_high) AND (Latency IS very_low) THEN (Network IS very_low)"
R_N23 = "IF (Output IS very_high) AND (Latency IS normal) THEN (Network IS low)"
R_N24 = "IF (Output IS very_high) AND (Latency IS high) THEN (Network IS low)"
R_N25 = "IF (Output IS very_high) AND (Latency IS very_high) THEN (Network IS low)"
...
R_F1 = "IF (Network IS very_low) AND (Load IS very_low) THEN (Result IS low)"
R_F2 = "IF (Network IS very_low) AND (Load IS low) THEN (Result IS low)"
...
R_F24 = "IF (Network IS very_high) AND (Load IS high) THEN (Result IS high)"
...
for x in np.linspace(0,1,DIVs):
    for y in np.linspace(0,1,DIVs):
        FS_N.set_variable("Latency", x)
        FS_N.set_variable("Output", y)
        tip = FS_N.Mamdani_inference()["Network"]
        xs.append(x)
        ys.append(y)
        zs.append(tip)
xs = np.array(xs)
...
ax.set_title("Fuzzy Network", pad=20)
ax.set_zlim(0, 1)
plt.tight_layout()
...
for x in np.linspace(0,1,DIVs):
    for y in np.linspace(0,1,DIVs):
        ...
        FS_L.set_variable("Processor", y)
        tip = FS_L.Mamdani_inference()["Load"]
        xs.append(x)
        ys.append(y)
        zs.append(tip)
xs = np.array(xs)
...
ax.set_zlabel("Result")
ax.set_title("Fuzzy Load", pad=20)
ax.set_zlim(0, 1)
plt.tight_layout()
...
for x in np.linspace(0,1,DIVs):
    for y in np.linspace(0,1,DIVs):
        ...
        FS_F.set_variable("Network", y)
        tip = FS_F.Mamdani_inference()["Result"]
        xs.append(x)
        ys.append(y)
        zs.append(tip)
xs = np.array(xs)
...
ax.set_zlabel("Result")
ax.set_title("Fuzzy final", pad=20)
ax.set_zlim(-1, 1)
plt.tight_layout()
